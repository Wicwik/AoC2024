...
for code in codes:
    ...
    prev_moves_per_code = get_moves(Pos(3,2), inv_numeric_keypad, get_counts(code))

    for i in range(25):
        moves_per_code = get_moves(Pos(0,2), inv_directional_keypad, prev_moves_per_code)
        prev_moves_per_code = moves_per_code

    l = 0
    for c, count in moves_per_code.items():
        l += len(c) * count
